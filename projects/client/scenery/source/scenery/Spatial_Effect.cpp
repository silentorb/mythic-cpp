...
namespace scenery {
  ...
  void Spatial_Effect::activate(mat4 &transform, float opacity) {
    Effect::activate();
...
//    }
    model_property.set_matrix(transform);

    if (opacity != this->opacity) {
      this->opacity = opacity;
      color_property.set_vec4(1, 1, 1, opacity);
    }
...
//    }
    normal_property.set_matrix(normal_transform);
