...
namespace haft {
  ...
  void Input_Manager::update() {
    if (!source)
      throw std::runtime_error("Input source not set.");

    previous_state = move(current_state);
    current_state = std::unique_ptr<Input_State>(source->get_input_state());
...
//    }
  ...
  void Input_Manager::add_input_source(Input_Source *new_source) {
    if (source)
      throw std::runtime_error("Multiple input sources not yet implemented.");

    source = std::unique_ptr<Input_Source>(new_source);
  }
}
